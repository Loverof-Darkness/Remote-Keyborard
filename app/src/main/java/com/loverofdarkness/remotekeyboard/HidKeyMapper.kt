...
object HidKeyMapper {
    ...
    private val letters = mapOf('a' to 0x04,'b' to 0x05,'c' to 0x06,'d' to 0x07,'e' to 0x08,'f' to 0x09,'g' to 0x0A,'h' to 0x0B,'i' to 0x0C,'j' to 0x0D,'k' to 0x0E,'l' to 0x0F,'m' to 0x10,'n' to 0x11,'o' to 0x12,'p' to 0x13,'q' to 0x14,'r' to 0x15,'s' to 0x16,'t' to 0x17,'u' to 0x18,'v' to 0x19,'w' to 0x1A,'x' to 0x1B,'y' to 0x1C,'z' to 0x1D)
    ...
    fun map(c: Char): Mapping? {
        letters[c.lowercaseChar()]?.let { return Mapping(it, if (c.isUpperCase()) MODIFIER_LEFT_SHIFT else 0) }
        digits[c]?.let { return Mapping(it) }
        plain[c]?.let { return Mapping(it) }
        shifted[c]?.let { return Mapping(map(it)?.usage ?: return null, MODIFIER_LEFT_SHIFT) }
        return null
    }

    fun usageFor(name: String): Int? {
        return when (name.trim().uppercase()) {
            "ENTER", "RETURN" -> ENTER; "BACKSPACE", "BKSP" -> BACKSPACE; "TAB" -> TAB
            "ESC", "ESCAPE" -> ESC; "DELETE", "DEL" -> DELETE; "INSERT", "INS" -> INSERT
            "UP" -> UP; "DOWN" -> DOWN; "LEFT" -> LEFT; "RIGHT" -> RIGHT
            "HOME" -> HOME; "END" -> END; "PGUP", "PAGEUP" -> PAGE_UP; "PGDN", "PAGEDOWN" -> PAGE_DOWN
            "CAPS", "CAPSLOCK" -> CAPS_LOCK; "PRINT", "PRINTSCREEN" -> PRINT_SCREEN; "SCROLL", "SCROLLLOCK" -> SCROLL_LOCK; "PAUSE" -> PAUSE
            else -> {
                val n = name.trim().uppercase().removePrefix("F").toIntOrNull()
                if (name.trim().uppercase().startsWith("F") && n != null && n in 1..12) 0x3A + n - 1
                else if (name.trim().length == 1) map(name.trim()[0])?.usage else null
            }
        }
    }
