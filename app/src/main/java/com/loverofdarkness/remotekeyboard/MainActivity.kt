...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        editor = EditText(this).apply {
            hint = "Tap here to open your native keyboard"; textSize = 18f; minLines = 5; maxLines = 12; gravity = Gravity.TOP or Gravity.START
    ...
    private fun startSearch() {
        ...
        try {
            if (a.isDiscovering) a.cancelDiscovery(); loadKnownDevices(); searchButton.isEnabled = false; searchButton.text = "Searching…"; status.text = "Searching nearby Bluetooth devices…"; a.startDiscovery()
    ...
    private fun connectSelected() {
        ...
        connectionEpoch.incrementAndGet(); clearLocalBuffer(); status.text = "Connecting to ${safeName(device)}…"; hid?.connect(device)
    }
        ...
        sender.execute {
            try {
                ...
                if (connectionEpoch.compareAndSet(epoch, epoch + 1)) runOnUiThread {
                    clearLocalBuffer(); editor.isEnabled = false; sendButton.isEnabled = false; status.text = "Sending stopped: ${e.message ?: "HID error"}"; ConnectionNotification.clear(this)
                }
            }
        }
        return true
    ...
    private fun Intent.getParcelableExtraCompat(): BluetoothDevice? = if (Build.VERSION.SDK_INT >= 33) getParcelableExtra(BluetoothDevice.EXTRA_DEVICE, BluetoothDevice::class.java) else getParcelableExtra(BluetoothDevice.EXTRA_DEVICE)
